...
class Validation():
    ...
    def plotSignalAverage(simResultArray, diffusionTimes, qPoints, signalFunc, plotTitle):
    ...
    def plotDelegator(data, diffusionTimes, xData, plotTitle, xDataType, yLabel, pValuePlot):
        if xDataType == "comparison":
            ...
            Validation.comparisonPlot(data, diffusionTimes, xData, plotTitle, yLabel)

        elif xDataType == "convergence":
            xLabel = "Number of Particles"
            Validation.convergencePlot(data, diffusionTimes, xData, plotTitle, yLabel)

        else:
            print("ERROR: invalid xDataType")
            return None
        ...
        if pValuePlot:
            ...
            Validation.pValuePlot(pValues, diffusionTimes, xData, plotTitle + plotTitleExtra, xLabel)

    def finalPlot(data, diffusionTimes, xData, plotTitle, dataType, xDataType, radius=None, D=None):
        if dataType == "signal":
            yLabel="RMSE between exact and empirical SGP signals"
            Validation.plotDelegator(data, diffusionTimes, xData, plotTitle, xDataType, yLabel, pValuePlot=False)

        elif dataType == "diffusion":
            yLabel="Supremum distance between exact and empirical CDFs"
            Validation.plotDelegator(data, diffusionTimes, xData, plotTitle, xDataType, yLabel, pValuePlot=False)

        elif dataType == "md":
            yLabel = "Norm of Mean Displacement"
            Validation.plotDelegator(data, diffusionTimes, xData, plotTitle, xDataType, yLabel, pValuePlot=True)

        elif dataType == "rmsd":
            newData = data.copy()
            for t in range(len(diffusionTimes)):
                #expectedMean = Util.RMSD_sphere(radius, D, diffusionTimes[t], 20)
                expectedMean = Util.RMSD_planes(radius, D, diffusionTimes[t], 20) #TEMPORARY
                newData[t, :, :] = newData[t, :, :] - expectedMean
            yLabel = "Difference between theoretical and simulated RMSD"
            Validation.plotDelegator(newData, diffusionTimes, xData, plotTitle, xDataType, yLabel, pValuePlot=True)
    ...
    def run1SimSignal(sim, printMessage, qPoints, trueSignalPoints, graphTitle, plotGraph):
        ...
        rmsd = (np.average( sim.getDisplacements()[:, 0]**2))**0.5 #TEMPORARY
    ...
    def runSphereSignalConv(nRuns, plotGraph, diffusionTimes, nStep, nParts, radius, D, T2, qPoints):
        results = np.zeros((len(diffusionTimes), len(nParts), 3, nRuns))
        ...
        for t in range(len(diffusionTimes)):
            ...
            for i in range(len(nParts)):
                for r in range(nRuns):
                    ...
                    graphTitle = "SGP signal attenuation in an impermeable sphere (sphere radius = {radius}um)\n"\
                                 "Diffusion time = {diffusionTime}ms, Number of particles = {nPart}, Number of steps = {n}, run {r}/{nRuns}"\
                        .format(radius=radius, diffusionTime=diffusionTime, nPart=nPart, n=nStep, r=r+1, nRuns=nRuns)
                    results[t, i, :, r] = Validation.run1SimSignal(sim, printMessage, qPoints, trueSignalPoints, graphTitle, plotGraph)
        return results

    def runPlanesSignalConv(nRuns, plotGraph, diffusionTimes, nStep, nParts, spacing, D, T2, qPoints):
        results = np.zeros((len(diffusionTimes), len(nParts), 3, nRuns))
        ...
        for t in range(len(diffusionTimes)):
            ...
            for i in range(len(nParts)):
                for r in range(nRuns):
                    ...
                    graphTitle = "SGP signal attenuation in an impermeable sphere (sphere radius = {radius}um)\n"\
                                 "Diffusion time = {diffusionTime}ms, Number of particles = {nPart}, Number of steps = {n}, run {r}/{nRuns}"\
                        .format(radius=7474, diffusionTime=diffusionTime, nPart=nPart, n=nStep, r=r+1, nRuns=nRuns)
                    results[t, i, :, r] = Validation.run1SimSignal(sim, printMessage, qPoints, trueSignalPoints, graphTitle, plotGraph)
        return results
