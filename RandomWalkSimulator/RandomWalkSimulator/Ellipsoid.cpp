...
void Ellipsoid::collide(std::shared_ptr<AbstractParticle3D> particle, Eigen::Vector3d oldPos, Eigen::Vector3d intersection, AbstractSimulation &sim) {
	...
	if (Util::getRandomUniform(0, 1) >= transmissionProba) {
		...
		if (contains(oldPos)) {
			...
			particle->changeCompartment(m_parentComp, sim.getTimeStep());
		}
		else {
			//we are entering this compartment
			particle->changeCompartment(std::make_shared<Ellipsoid>(*this), sim.getTimeStep());
		}
	}
}
