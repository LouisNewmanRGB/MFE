...
void Sphere::collide(std::shared_ptr<AbstractParticle3D> particle, Eigen::Vector3d oldPos, Eigen::Vector3d intersection, AbstractSimulation &sim) {
	...
	if (Util::getRandomUniform(0, 1) >= transmissionProba) {
		...
		if (contains(oldPos)) {
			...
			particle->changeCompartment(std::make_shared<Sphere>(*this), sim.getTimeStep());
		}
	}
}
