...
Environment::Environment(double T2, double diffusivity, double sizeX, double sizeY, double sizeZ)
	...
	m_tolerance{ std::max({sizeX, sizeY, sizeZ})*Simulation::m_TOL}
{
}
...
bool Environment::collide(std::shared_ptr<AbstractParticle3D> particle, Eigen::Vector3d oldPos, Eigen::Vector3d intersection, double timeStep) {
